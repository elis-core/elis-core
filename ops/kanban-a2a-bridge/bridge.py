...
# --- Bridge auth ---
# /kanban/create is a general-purpose production write (any of the 5 primary
# profiles, any title/body/priority). profiles/hermes/elis-pm/ENVIRONMENT.md
# documents this bridge as read-only for sandboxed PM ("Allowed operations:
# /health, /kanban/identity, /kanban/status-counts, /a2a/status" ... "PM must
# never write directly to any Kanban database. Real task assignment requires
# a separate controlled canary validating Kanban write and A2A message
# delivery.") — that governed canary/approval never happened for this
# endpoint, so it stays hard-disabled unless explicitly opted into with a
# configured token. The bounded canary endpoints (kanban/canary-*, a2a/canary*)
# were validated in docs/hermes-migration/closeouts/
# sb-elis-supervisor-bridge-mediated-a2a-2026-07-09.md and stay reachable
# without a token for backward compatibility, but will enforce one if set.
BRIDGE_AUTH_TOKEN = os.environ.get("ELIS_BRIDGE_TOKEN")
ALLOW_KANBAN_CREATE = os.environ.get("ELIS_BRIDGE_ALLOW_KANBAN_CREATE") == "1"
...
def _token_ok(handler):
    if not BRIDGE_AUTH_TOKEN:
        return False
    supplied = _bearer_token(handler) or ""
    return hmac.compare_digest(supplied, BRIDGE_AUTH_TOKEN)
...
class Handler(BaseHTTPRequestHandler):
    ...
    MUTATING_PATHS = {
        "/a2a/canary-send", "/a2a/canary",
        "/kanban/canary-create", "/kanban/canary-comment", "/kanban/canary-close",
    }
    ...
    def do_POST(self):
        ...
        try:
            ...
            if path == "/kanban/create":
                if not ALLOW_KANBAN_CREATE or not _token_ok(self):
                    return json_response(self, 403, {
                        ...
                        "error": "not_governed",
                        "message": (
                            "POST /kanban/create is disabled by default. "
                            "profiles/hermes/elis-pm/ENVIRONMENT.md documents this "
                            "bridge as read-only for sandboxed PM and states real "
                            "task assignment requires a separate controlled canary "
            ...
            if path in self.MUTATING_PATHS and BRIDGE_AUTH_TOKEN and not _token_ok(self):
...
if __name__ == "__main__":
    ...
    print(f"kanban/create: {'ENABLED (token required)' if ALLOW_KANBAN_CREATE and BRIDGE_AUTH_TOKEN else 'disabled (not governed, see ENVIRONMENT.md)'}", flush=True)
    print(f"canary endpoints: {'token required' if BRIDGE_AUTH_TOKEN else 'WARNING: no ELIS_BRIDGE_TOKEN configured, running without caller auth'}", flush=True)
